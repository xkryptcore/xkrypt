...
// Feb, 2018 - Biznatch Enterprises & XKrypt (XKY)
...
// * Function: LoadFirewallSettings (xky.conf)*
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "XKY " + FormatFullVersion();
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. XKY is probably already running."), addrBind.ToString());
        else
