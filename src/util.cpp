...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\XKY
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\XKY
    // Mac: ~/Library/Application Support/XKY
    // Unix: ~/.XKY
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "XKY";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "xky.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty xky.conf if it does not exist
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "xkyd.pid"));
