...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: xky.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: xkyd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeXKYamount=<n> " + _("Keep N XKY anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. XKY is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. XKY is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("XKY version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "XKY server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of XKY") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart XKY to complete") << "\n";
    ...
    nAnonymizeXKYAmount = GetArg("-anonymizeXKYamount", 0);
    if(nAnonymizeXKYAmount > 999999) nAnonymizeXKYAmount = 999999;
    if(nAnonymizeXKYAmount < 2) nAnonymizeXKYAmount = 2;
    ...
    LogPrintf("Anonymize XKY Amount %d\n", nAnonymizeXKYAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1XKY+1000 == (.1XKY+100)*10
       10XKY+10000 == (1XKY+1000)*10
    */
