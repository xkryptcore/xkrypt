...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeXKYAmount"))
        settings.setValue("nAnonymizeXKYAmount", 1000);
    nAnonymizeXKYAmount = settings.value("nAnonymizeXKYAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeXKYAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeXKYAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeXKYAmount:
            return QVariant(nAnonymizeXKYAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeXKYAmount:
            nAnonymizeXKYAmount = value.toInt();
            settings.setValue("nAnonymizeXKYAmount", nAnonymizeXKYAmount);
            emit AnonymizeXKYAmountChanged(nAnonymizeXKYAmount);
            break;
        default:
            break;
        }
