...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeXKYAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeXKYAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeXKYAmount = strAnonymizeXKYAmount.remove(strAnonymizeXKYAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeXKYAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeXKYAmount*COIN) nMaxToAnonymize = nAnonymizeXKYAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeXKYAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(nAnonymizeXKYAmount));
        strAnonymizeXKYAmount = strAnonymizeXKYAmount.remove(strAnonymizeXKYAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeXKYAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
